...
class NW1
  ...
  def initialize(seq1, seq2, gap = -1)
    ...
    @matrix_score = Array.new(@seq1.length + 1) { Array.new(@seq2.length + 1, 0) }
    @matrix_trace = Array.new(@seq1.length + 1) { Array.new(@seq2.length + 1, :end) }
  end
    ...
    (0...(seq1.length)).each do |yy|
      (0...(seq2.length)).each do |xx|
        if yy.zero? && xx.zero?
          ...
          @matrix_trace[yy][xx] = :end
        elsif yy.zero?
          @matrix_score[yy][xx] = -1 * xx
          @matrix_trace[yy][xx] = :top
        elsif xx.zero?
          @matrix_score[yy][xx] = -1 * yy
          @matrix_trace[yy][xx] = :left
        else
          ...
          @matrix_trace[yy][xx] = :left if l_score == score
          @matrix_trace[yy][xx] = :top if t_score == score
          @matrix_trace[yy][xx] = :diagonal if c_score == score
        end
      end
    end
  end
  ...
  def get_trace
    ...
    y = seq1.length - 1
    x = seq2.length - 1
    y, x = get_trace_branch(y, x, @data, seq1, seq2, @matrix_trace[y][x]) while @matrix_trace[y][x] != :end
    @data.each_value(&:reverse!)
  end
end
